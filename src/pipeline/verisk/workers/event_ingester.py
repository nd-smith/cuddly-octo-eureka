...
Downstream consumers of enrichment.pending:
- EnrichmentWorker: creates download tasks and executes plugins
- DeltaEventsWorker: extracts raw_event and writes to Delta Lake
...
class EventIngesterWorker:
    ...
    def _adjust_batch_size(self, batch_record_count: int) -> None:
        ...
        A full batch (>= REALTIME_BATCH_SIZE) indicates the worker is behind
        and should scale up. A partial batch means it's caught up.
        """
        if batch_record_count >= self.REALTIME_BATCH_SIZE:
