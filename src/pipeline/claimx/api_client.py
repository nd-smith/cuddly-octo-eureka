...
class ClaimXApiClient:
    ...
    def __del__(self) -> None:
        if hasattr(self, "_closed") and not self._closed:
            warnings.warn(
                "ClaimXApiClient was not properly closed. "
                "Use async context manager: 'async with ClaimXApiClient(...) as client:' "
                "or call await client.close() explicitly.",
                ResourceWarning,
                stacklevel=2,
            )
            ...
            logger.warning(
                ...
                extra={
                    **ctx,
                    ...
                    "http_url": url,
                    "circuit_state": "open",
                    "delay_seconds": round(retry_after, 1),
                },
            )
            raise error
