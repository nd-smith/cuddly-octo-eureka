...
Writes events to claimx_events with async support. Deduplication handled upstream
by the event ingester before messages reach the enrichment_pending topic.
"""
...
class ClaimXEventsDeltaWriter(BaseDeltaWriter):
    ...
    async def write_events(self, events: list[dict[str, Any]]) -> bool:
        ...
        Args:
            events: List of event dicts from ClaimXEnrichmentTask.model_dump(include=...):
