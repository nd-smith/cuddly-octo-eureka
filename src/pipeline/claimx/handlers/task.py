...
class TaskHandler(EventHandler):
    ...
    When a task_event_producer is configured, also produces a ClaimXTaskEvent
    message combining project, task, and report data for downstream consumers.
    """
    ...
    def _build_task_rows(
        ...
        # Carry form response groups for downstream routing decisions (e.g. itel filter)
        task_row["_form_response_groups"] = (response.get("response") or {}).get("groups", [])
    ...
    async def _produce_task_event(
        ...
        task_event = ClaimXTaskEvent.model_validate(task_event_data)

        await self.task_event_producer.send(
            value=task_event,
            key=event.trace_id,
        ...
        logger.info(
            ...
            extra={
                ...
                "task_assignment_id": task_event.task_assignment_id,
                "project_id": event.project_id,
                "report_id": report_id,
