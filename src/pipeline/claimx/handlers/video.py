...
class VideoCollabHandler(EventHandler):
    ...
    async def _produce_video_event(
        self,
        collab_data: dict,
        event: ClaimXEventMessage,
    ) -> None:
        """Produce a ClaimXVideoCollabEvent to EventHub."""
        video_event_data = transformers.build_video_event(
            collab_data=collab_data,
            trace_id=event.trace_id,
            event_type=event.event_type,
        )
        video_event = ClaimXVideoCollabEvent.model_validate(video_event_data)

        await self.video_event_producer.send(
            value=video_event,
            key=event.trace_id,
        ...
        logger.info(
            "Produced video collab event",
            extra={
                "handler_name": VideoCollabHandler.HANDLER_NAME,
                "video_collaboration_id": collab_data.get("videoCollaborationId"),
                "project_id": event.project_id,
                **extract_log_context(event),
            },
        )
