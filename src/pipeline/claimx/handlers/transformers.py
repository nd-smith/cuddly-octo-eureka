...
def _extract_mfn(project_response: dict[str, Any]) -> str | None:
    """Extract MFN from project export response.

    Priority: data.project.masterFileName, then first non-null teamMember mfn.
    """
    inner = project_response.get("data", project_response)
    project = inner.get("project", {})

    mfn = safe_str(project.get("masterFileName"))
    if mfn:
        return mfn

    for member in inner.get("teamMembers", []):
        member_mfn = safe_str(member.get("mfn"))
        if member_mfn:
            return member_mfn
...
def build_task_event(
    ...
    media_response: list[dict[str, Any]] | None,
    trace_id: str,
    event_type: str,
    project_id: str,
) -> dict[str, Any]:
    ...
    if media_response:
        ...
        if media_item:
            ...
            media_id = safe_str_id(media_item.get("mediaID"))

    return {
        "trace_id": trace_id,
        "event_type": event_type,
        "project_id": project_id,
        "claim_number": safe_str(project.get("projectNumber")),
        "project_status": safe_str(project.get("status")),
        "customer_firstname": safe_str(customer.get("firstName")),
        "customer_lastname": safe_str(customer.get("lastName")),
        "loss_address_street": safe_str(address.get("street1")),
        "loss_address_city": safe_str(address.get("city")),
        ...
        "task_status": safe_str(task_response.get("status")),
...
def build_video_event(
    ...
    """Build a ClaimXVideoCollabEvent dict from the video collaboration API response."""
    raw_email = safe_str(collab_data.get("emailUserName")) or ""
    ntid = raw_email.strip().split("@")[0] or None

    return {
        "trace_id": trace_id,
        "event_type": event_type,
        "project_id": safe_str(collab_data.get("claimId")),
        "mfn": safe_str(collab_data.get("mfn")),
        "ntid": ntid,
        "claim_number": safe_str(collab_data.get("claimNumber")),
        "policy_number": safe_str(collab_data.get("policyNumber")),
        "number_of_videos": safe_int(collab_data.get("numberOfVideos")),
        "number_of_photos": safe_int(collab_data.get("numberOfPhotos")),
        "total_time_seconds": safe_decimal_str(collab_data.get("totalTimeSeconds")),
        "session_count": safe_int(collab_data.get("sessionCount")),
        "total_time": safe_str(collab_data.get("totalTime")),
    }
