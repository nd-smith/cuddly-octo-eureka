...
class ClaimXEventMessage(BaseModel):
    ...
    def _build_trace_id(
        ...
        ts = ingested_at.isoformat() if isinstance(ingested_at, datetime) else str(ingested_at or "")
        parts = [project_id, event_type, ts]
    ...
    def from_raw_event(cls, row: dict[str, Any]) -> "ClaimXEventMessage":
        ...
        raw_media_ids = cls._get_field(row, "media_ids", "mediaIds")
        media_ids = [str(m) for m in raw_media_ids] if raw_media_ids else None
