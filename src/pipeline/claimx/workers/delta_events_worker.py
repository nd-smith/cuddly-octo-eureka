...
Writes enrichment task messages to Delta Lake with batch processing.
Consumes from the enrichment_pending topic (downstream of deduplication).
"""
...
class ClaimXDeltaEventsWorker:
    """
    Worker to consume deduplicated ClaimX enrichment tasks and write them to Delta Lake.

    Consumes ClaimXEnrichmentTask records from the enrichment_pending topic
    (downstream of the event ingester's deduplication) and writes them to
    the claimx_events Delta table.
    ...
    async def _handle_event_message(self, record: PipelineMessage) -> None:
        ...
        try:
            task = ClaimXEnrichmentTask(**message_data)
            event_data = task.model_dump(
                include={"trace_id", "event_type", "project_id", "media_id",
                         "task_assignment_id", "video_collaboration_id",
                         "master_file_name", "ingested_at"},
            )
        except ValidationError as e:
            logger.exception("Failed to parse ClaimXEnrichmentTask", extra=self._record_log_context(record))
            raise PermanentError(str(e)) from e
