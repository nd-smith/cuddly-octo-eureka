...
def _reset_delta_process_pool() -> None:
    """Replace a broken/stuck process pool with a fresh one.

    Forcefully kills alive worker processes before shutting down the pool
    so that subprocesses stuck on storage I/O are not leaked.  Without this,
    ``shutdown(wait=False)`` leaves the workers running indefinitely, the old
    processes hold storage connections/locks, and new pools inherit the same
    resource-exhaustion problem.
    """
    global _process_pool
    if _process_pool is not None:
        pool = _process_pool
        _process_pool = None  # allow new pool creation immediately
        try:
            # Kill stuck workers first — they won't respond to the normal
            # shutdown sentinel because they're blocked on storage I/O.
