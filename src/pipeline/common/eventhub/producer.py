...
class EventHubProducer:
    ...
    def __del__(self) -> None:
        if hasattr(self, "_started") and self._started:
            warnings.warn(
                f"EventHubProducer '{self.eventhub_name}' was not properly stopped. "
                "Call await producer.stop() explicitly.",
                ResourceWarning,
                stacklevel=2,
            )
    ...
    async def send(
        ...
        try:
            ...
            try:
                ...
                record_producer_error(self.eventhub_name, "MessageTooLarge")
                logger.error(
                    "Message exceeds Event Hub size limit",
                    extra={
                        "entity": self.eventhub_name,
                        "key": key,
                        "value_size": len(value_bytes),
                ...
                raise ValueError(
                    f"Message too large for Event Hub: {len(value_bytes)} bytes. "
                    f"Original error: {ve}"
                ) from ve
    ...
    async def _fill_batches(
        self,
        messages: "list[tuple[str, BaseModel | bytes]]",
        headers: dict[str, str] | None = None,
    ) -> tuple[list, int]:
        ...
        for key, value in messages:
            ...
            try:
                ...
                if batch.size_in_bytes == 0:
                    # Single event exceeds the batch limit even on an empty batch
                    logger.error(
                        "Single event exceeds Event Hub batch size limit, skipping",
                        extra={
                            "key": key,
                            "value_size": len(value_bytes),
                        },
                ...
                try:
                    ...
                    # Single event exceeds limit even on empty batch
                    logger.error(
                        "Single event exceeds Event Hub batch size limit, skipping",
                        extra={
                            "key": key,
