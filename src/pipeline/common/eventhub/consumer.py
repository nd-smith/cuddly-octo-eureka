...
class EventHubConsumerOptions:
    ...
    owner_level: int = 0
    on_partition_revoked: Callable[[str], Awaitable[None]] | None = None
...
class EventHubConsumer:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._on_partition_revoked_callback = options.on_partition_revoked
    ...
    def __del__(self) -> None:
        if hasattr(self, "_running") and self._running:
            warnings.warn(
                f"EventHubConsumer '{self.eventhub_name}' was not properly stopped. "
                "Call await consumer.stop() explicitly.",
                ResourceWarning,
                stacklevel=2,
            )
    ...
    async def _on_partition_close(self, partition_context, reason):
        ...
        # Invoke revocation callback BEFORE clearing state so the callback
        # can still flush work and commit while partition context is valid.
        if self._on_partition_revoked_callback is not None:
            try:
                logger.info(
                    "Partition revoked - flushing pending work",
                    extra={
                        "partition_id": partition_id,
                        "reason": reason,
                    },
                )
                await self._on_partition_revoked_callback(partition_id)
            except Exception:
                logger.error(
                    "Error in partition revoked callback",
    ...
    def _build_dlq_entity_map(self) -> dict:
        ...
        Maps:
        - verisk source (xactanalysis) -> verisk-dlq
        - claimx source (claimxperience) -> claimx-dlq
        - itel cabinet topics -> pcesdopodappv1-itel-api-errors
        """
        return {
            # Verisk domain — source EventHub
            "xactanalysis": "verisk-dlq",
            # ClaimX domain — source EventHub
            "claimxperience": "claimx-dlq",
            # iTel Cabinet — pending + completed consumers
            "pcesdopodappv1-itel-cabinet-pending": "pcesdopodappv1-itel-api-errors",
            "pcesdopodappv1-itel-cabinet-completed": "pcesdopodappv1-itel-api-errors",
        }
