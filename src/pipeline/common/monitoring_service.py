...
Usage:
    python -m pipeline.common.monitoring_service --prometheus-url http://localhost:9090
"""
...
class PrometheusClient:
    ...
    def __del__(self) -> None:
        if hasattr(self, "_session") and self._session and not self._session.closed:
            warnings.warn(
                "PrometheusClient was not properly closed. "
                "Call await client.close() explicitly.",
                ResourceWarning,
                stacklevel=2,
            )
