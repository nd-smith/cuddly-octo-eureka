...
class ToDictMixin:
    """Shared to_dict() for dataclasses with datetime serialization."""
...
class TaskEvent:
    ...
    # Optional media filter (from ClaimX event)
    ...
    def from_message(cls, raw: dict) -> "TaskEvent":
        ...
        try:
            return cls(
                ...
                media_ids=raw.get("media_ids") or raw.get("mediaIds"),
            )
...
@dataclass
class CabinetSubmission(ToDictMixin):
    """
    ...
    lower_cabinets_lf: int | None = None
    ...
    upper_cabinets_lf: int | None = None
    ...
    full_height_cabinets_lf: int | None = None
    ...
    island_cabinets_lf: int | None = None
    ...
    def __post_init__(self):
        """Ceil-round any float values assigned to int-typed fields."""
        _int_fields = {
            "assignment_id",
            "task_id",
        ...
        for name in _int_fields:
            ...
            if isinstance(val, float):
                setattr(self, name, round(val))


@dataclass
class CabinetAttachment(ToDictMixin):
    """
