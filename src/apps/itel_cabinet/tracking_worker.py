...
Configuration:
    apps/itel_cabinet/config/workers.yaml
...
CONNECTIONS_CONFIG_PATH = CONFIG_DIR / "claimx.yaml"
...
class ItelCabinetTrackingWorker:
    ...
    async def start(self):
        ...
        initialize_worker_telemetry("plugins", "itel-cabinet")
        set_log_context(stage="tracking", worker_id="itel-cabinet")
...
async def build_tracking_worker(
    ...
    Returns:
        Tuple of (worker, connection_manager, api_sender) for lifecycle management.
    ...
    # Build the API sender (loads iTel API config, creates producers, adds connections)
    from .api_sender import build_api_sender

    api_sender = await build_api_sender(bootstrap_servers, connection_manager)
    await api_sender.start()
    ...
    return worker, connection_manager, api_sender
