...
class TestApplySslDevBypass:
    ...
    @patch.dict(os.environ, {"DISABLE_SSL_VERIFY": "1", "ENVIRONMENT": "dev"}, clear=True)
    def test_accepts_1_as_true(self):
        apply_ssl_dev_bypass()
        assert ssl_bypass_module._patched is True

    @patch.dict(os.environ, {"DISABLE_SSL_VERIFY": "yes", "ENVIRONMENT": "dev"}, clear=True)
    def test_accepts_yes_as_true(self):
        apply_ssl_dev_bypass()
        assert ssl_bypass_module._patched is True

    @patch.dict(os.environ, {"DISABLE_SSL_VERIFY": "TRUE", "ENVIRONMENT": "dev"}, clear=True)
