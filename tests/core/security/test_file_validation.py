...
class TestIsAllowedContentType:
    ...
    def test_allowed_office_content_types(self):
        """Should accept Microsoft Office content types."""
        assert is_allowed_content_type("application/msword") is True
        assert is_allowed_content_type(
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        ) is True
        assert is_allowed_content_type("application/vnd.ms-excel") is True
        assert is_allowed_content_type(
...
class TestValidateFileType:
    ...
    def test_valid_docx_with_content_type(self):
        """Should accept DOCX with matching Content-Type."""
        validate_file_type(
            "report.docx",
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        )  # Should not raise

    def test_valid_xlsx_with_content_type(self):
        """Should accept XLSX with matching Content-Type."""
        validate_file_type(
            "data.xlsx",
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        )  # Should not raise

    def test_valid_doc_with_content_type(self):
        """Should accept DOC with matching Content-Type."""
        validate_file_type("report.doc", "application/msword")  # Should not raise

    def test_valid_xls_with_content_type(self):
        """Should accept XLS with matching Content-Type."""
        validate_file_type(
