...
# =============================================================================
# EventHubProducer - oversized message handling
# =============================================================================


class TestEventHubProducerOversizedMessages:
    def _make_started_producer(self):
        from pipeline.common.eventhub.producer import EventHubProducer
    ...
    @patch("pipeline.common.eventhub.producer.record_message_produced")
    @patch("pipeline.common.eventhub.producer.EventData")
    async def test_send_raises_clear_error_for_oversized_message(
        self, MockEventData, mock_record, mock_error
    ):
        producer, mock_client = self._make_started_producer()
        mock_event = MagicMock()
        mock_event.properties = {}
        MockEventData.return_value = mock_event
        ...
        mock_batch.add.side_effect = ValueError(
            "EventDataBatch has reached its size limit: 1048576"
        )
        mock_client.create_batch = AsyncMock(return_value=mock_batch)

        with pytest.raises(ValueError, match="Message too large for Event Hub"):
            await producer.send(value=b"x" * 2_000_000)

        mock_error.assert_called_once_with("my-entity", "MessageTooLarge")

    @patch("pipeline.common.eventhub.producer.record_message_produced")
    @patch("pipeline.common.eventhub.producer.message_processing_duration_seconds")
    @patch("pipeline.common.eventhub.producer.EventData")
    async def test_fill_batches_skips_oversized_single_event(
        self, MockEventData, mock_duration, mock_record
    ):
        from pipeline.common.eventhub.producer import EventHubProducer
        ...
        # First batch: first add succeeds, second raises (oversized on empty)
        batch1 = MagicMock()
        batch1.size_in_bytes = 100  # Non-zero, batch has content

        # Second batch is empty, single event too large
        batch2 = MagicMock()
        batch2.size_in_bytes = 0  # Empty batch

        call_count = 0

        def batch1_add(event_data):
            nonlocal call_count
            call_count += 1
            if call_count > 1:
                raise ValueError("EventDataBatch has reached its size limit")

        batch1.add.side_effect = batch1_add
        batch2.add.side_effect = ValueError("EventDataBatch has reached its size limit")
        ...
        m1 = MagicMock()
        m1.model_dump_json.return_value = '{"small": true}'
        m2 = MagicMock()
        m2.model_dump_json.return_value = '{"huge": true}'

        # Should not raise — oversized event is skipped
        results = await producer.send_batch(messages=[("k1", m1), ("k2", m2)])
