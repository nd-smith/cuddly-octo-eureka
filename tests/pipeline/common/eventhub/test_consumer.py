...
class TestDLQEntityMapping:
    ...
    def test_build_dlq_entity_map_has_verisk_mapping(self):
        consumer = self._make_consumer()
        assert consumer._dlq_entity_map["xactanalysis"] == "verisk-dlq"

    def test_build_dlq_entity_map_has_claimx_mapping(self):
        consumer = self._make_consumer()
        assert consumer._dlq_entity_map["claimxperience"] == "claimx-dlq"

    def test_build_dlq_entity_map_has_itel_cabinet_mapping(self):
        consumer = self._make_consumer()
        assert consumer._dlq_entity_map["pcesdopodappv1-itel-cabinet-pending"] == "pcesdopodappv1-itel-api-errors"
        assert consumer._dlq_entity_map["pcesdopodappv1-itel-cabinet-completed"] == "pcesdopodappv1-itel-api-errors"

    def test_get_dlq_entity_name_returns_mapped_entity(self):
        consumer = self._make_consumer()
        assert consumer._get_dlq_entity_name("xactanalysis") == "verisk-dlq"
