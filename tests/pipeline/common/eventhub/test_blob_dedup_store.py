...
cleanup_expired, list_recent_keys, and close.
"""
...
class TestBlobDedupStoreCheckDuplicate:
    ...
    async def test_returns_true_for_valid_duplicate_via_metadata(self):
        """New-format blob: timestamp in blob metadata -> HEAD request only."""
        ...
        # Should NOT have called download_blob
        mock_blob_client.download_blob.assert_not_called()

    async def test_returns_true_for_valid_duplicate_via_content_fallback(self):
        """Pre-migration blob: no metadata -> falls back to content download."""
        store = self._make_store()

        stored_data = {"event_id": "evt1", "timestamp": time.time() - 10}
        mock_blob_client = MagicMock()
        mock_properties = MagicMock()
        mock_properties.metadata = {}  # No metadata (pre-migration)
        mock_blob_client.get_blob_properties = AsyncMock(return_value=mock_properties)
        ...
        assert is_dup is True
        assert metadata["event_id"] == "evt1"
        # Should have called download_blob as fallback
        mock_blob_client.download_blob.assert_awaited_once()

    async def test_returns_false_for_expired_entry_via_metadata(self):
        store = self._make_store()

        mock_blob_client = MagicMock()
        mock_properties = MagicMock()
        mock_properties.metadata = {"timestamp": str(time.time() - 7200)}
        mock_blob_client.get_blob_properties = AsyncMock(return_value=mock_properties)
        store._container.get_blob_client.return_value = mock_blob_client
    ...
    async def test_returns_false_for_expired_entry_via_content_fallback(self):
        store = self._make_store()
    ...
    async def test_returns_false_on_generic_error(self):
        ...
        mock_blob_client = MagicMock()
        mock_blob_client.get_blob_properties = AsyncMock(side_effect=RuntimeError("network error"))
        store._container.get_blob_client.return_value = mock_blob_client
    ...
    async def test_returns_false_when_metadata_none(self):
        """Blob exists but properties.metadata is None."""
        store = self._make_store()
...
class TestBlobDedupStoreMarkProcessed:
    ...
    async def test_uploads_blob_with_metadata_headers(self):
        store = self._make_store()
        ...
        # Verify blob metadata is passed for HEAD-based checks
        assert call_args[1]["metadata"]["timestamp"] == "12345"
        assert call_args[1]["metadata"]["event_id"] == "evt1"
    ...
    async def test_metadata_without_event_id(self):
        """When metadata has no event_id, blob metadata should only contain timestamp."""
        store = self._make_store()
        ...
        await store.mark_processed("worker", "key1", {"timestamp": 99999})

        call_args = mock_blob_client.upload_blob.call_args
        blob_meta = call_args[1]["metadata"]
        assert blob_meta == {"timestamp": "99999"}
        assert "event_id" not in blob_meta


# =============================================================================
# BlobDedupStore - list_recent_keys
# =============================================================================


class TestBlobDedupStoreListRecentKeys:
    def _make_store(self):
        from pipeline.common.eventhub.blob_dedup_store import BlobDedupStore
    ...
    async def test_returns_empty_when_container_not_initialized(self):
        from pipeline.common.eventhub.blob_dedup_store import BlobDedupStore

        store = BlobDedupStore(connection_string="conn", container_name="dedup")
        store._container = None

        result = await store.list_recent_keys("worker", 3600, 1000)
        assert result == []

    async def test_returns_recent_keys(self):
        store = self._make_store()

        recent_blob = MagicMock()
        recent_blob.name = "worker/trace-abc.json"
        recent_blob.last_modified = datetime.now(UTC)

        old_blob = MagicMock()
        old_blob.name = "worker/trace-old.json"
        old_blob.last_modified = datetime(2020, 1, 1, tzinfo=UTC)
    ...
    async def test_respects_max_keys(self):
        ...
        for i in range(10):
            blob = MagicMock()
            blob.name = f"worker/trace-{i}.json"
            blob.last_modified = datetime.now(UTC)
            blobs.append(blob)
        ...
        result = await store.list_recent_keys("worker", 3600, 3)

        assert len(result) == 3

    async def test_skips_blobs_without_last_modified(self):
        store = self._make_store()

        blob = MagicMock()
        blob.name = "worker/trace-x.json"
    ...
    async def test_handles_list_error(self):
        store = self._make_store()
