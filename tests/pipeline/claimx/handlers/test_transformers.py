...
# ============================================================================
# build_task_event — form_response stringification
# ============================================================================


def _make_form_response():
    """Create a realistic form response dict matching ClaimX API structure."""
    return {
        "countryEnum": None,
        "companyLogoUrl": None,
        "taskName": None,
        ...
        "groups": [
            {
                ...
                "questionAndAnswers": [
                    {
                        "questionText": "Describe the damage",
                        "responseAnswerExport": {"type": "text", "text": "Water damage"},
                        "component": "textArea",
                        "formControl": {"id": "ctrl-1"},
                    }
                ],
            }
        ],
...
class TestBuildTaskEventFormResponse:
    def test_form_response_is_json_string_when_present(self):
        ...
        parsed = json.loads(result["form_response"])
        assert parsed["groups"][0]["name"] == "General"

    def test_form_response_is_none_when_response_key_missing(self):
        task_response = {
            "assignmentId": 100,
            "taskId": 200,
            "status": "ASSIGNED",
        ...
        assert result["form_response"] is None

    def test_form_response_is_none_when_response_is_none(self):
        task_response = {
            "assignmentId": 100,
            "taskId": 200,
            "status": "ASSIGNED",
    ...
    def test_form_response_roundtrips_through_pydantic_model(self):
        """Verify form_response stays a string through Pydantic serialization."""
        task_response = {
            "assignmentId": 100,
            "taskId": 200,
            "taskName": "Inspection",
        ...
        model = ClaimXTaskEvent.model_validate(event_data)
        assert isinstance(model.form_response, str)
...
class TestClaimXTaskEventFormResponseValidator:
    def test_validator_stringifies_dict_input(self):
        """Validator converts raw dict to JSON string."""
        ...
        parsed = json.loads(event.form_response)
        assert parsed["groups"][0]["name"] == "General"

    def test_validator_preserves_string_input(self):
        """Validator leaves already-stringified JSON alone."""
        json_str = json.dumps(_make_form_response())
        event = ClaimXTaskEvent(
            trace_id="t1",
            event_type="CUSTOM_TASK_COMPLETED",
            project_id="123",
    ...
    def test_validator_stringifies_list_input(self):
        """Validator converts list to JSON string."""
        event = ClaimXTaskEvent(
            trace_id="t1",
            event_type="CUSTOM_TASK_COMPLETED",
            project_id="123",
            form_response=[{"name": "group1"}],
        )
        assert isinstance(event.form_response, str)
        assert json.loads(event.form_response) == [{"name": "group1"}]
