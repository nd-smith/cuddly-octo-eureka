...
class TestResultProcessorDeltaWrites:
    ...
    async def test_flush_batch_writes_to_delta(
        ...
        # Flush batch (starts background task)
        await processor._flush_batch()
        # Resolve the background write to complete the cycle
        await processor._resolve_pending_write()
...
class TestDoubleBufferWrites:
    """Test double-buffer write behavior for overlapping consumption and writes."""

    @pytest.mark.asyncio
    async def test_consumption_continues_during_write(
        self, mock_config, mock_producer, sample_success_message
    ):
        """Messages accumulate in the fresh batch while a background write runs."""
        processor = ResultProcessor(
            config=mock_config,
            producer=mock_producer,
            inventory_table_path="abfss://test/xact_attachments",
        ...
        # Use an event to control when the write completes
        ...
        processor._inventory_writer = AsyncMock()
        processor._inventory_writer.write_results = AsyncMock(side_effect=slow_write)
        processor._consumer = AsyncMock()
        processor._consumer.commit = AsyncMock()

        # Fill batch to trigger flush (batch_size=3)
        for _ in range(3):
            await processor._handle_result(sample_success_message)

        # Write should have started in background
        await write_started.wait()

        # Batch was cleared — new messages go into fresh batch
        assert len(processor._batch) == 0
        assert processor._pending_write_task is not None

        # Continue consuming while write is in-flight
        await processor._handle_result(sample_success_message)
        await processor._handle_result(sample_success_message)
        assert len(processor._batch) == 2  # Accumulated in fresh batch
        ...
        assert processor._batches_written == 1
        assert processor._total_records_written == 3
        # Second batch still accumulating
        assert len(processor._batch) == 2

    @pytest.mark.asyncio
    async def test_backpressure_when_previous_write_still_running(
        self, mock_config, mock_producer, sample_success_message
    ):
        """Second flush blocks until the first background write completes."""
        processor = ResultProcessor(
            config=mock_config,
            producer=mock_producer,
            inventory_table_path="abfss://test/xact_attachments",
        ...
        # First batch triggers flush (background task starts)
        await processor._handle_result(sample_success_message)
        await processor._handle_result(sample_success_message)
        assert processor._pending_write_task is not None

        # Second batch fills up — flush will block on _resolve_pending_write
        # Run in a task so we can check it blocks
        second_flush_done = asyncio.Event()

        async def fill_second_batch():
            await processor._handle_result(sample_success_message)
            await processor._handle_result(sample_success_message)
            second_flush_done.set()

        task = asyncio.create_task(fill_second_batch())

        # Give the event loop a chance to process
        await asyncio.sleep(0.05)
        # Second flush should be blocked (waiting on first write)
        assert not second_flush_done.is_set()

        # Release first write — second flush should proceed
        write_release.set()
        await task
        ...
        await processor._resolve_pending_write()
        assert processor._batches_written == 2

    @pytest.mark.asyncio
    async def test_shutdown_flushes_pending_write_and_remaining_batch(
        self, mock_config, mock_producer, sample_success_message
    ):
        """Shutdown resolves in-flight write and flushes remaining partial batch."""
        processor = ResultProcessor(
            config=mock_config,
            producer=mock_producer,
            inventory_table_path="abfss://test/xact_attachments",
        ...
        # Background write is pending
        assert processor._pending_write_task is not None

        # Add partial second batch (not enough to trigger flush)
        await processor._handle_result(sample_success_message)
        assert len(processor._batch) == 1

        # Shutdown should resolve pending + flush remaining
        await processor._flush_pending_on_shutdown()

        # Both batches written
        assert processor._batches_written == 2
        assert processor._total_records_written == 4
        assert len(processor._batch) == 0
        assert processor._pending_write_task is None

    @pytest.mark.asyncio
    async def test_resolve_pending_write_handles_exception(
        self, mock_config, mock_producer, sample_success_result
    ):
        """Background write exception is caught and routed to retry handler."""
        processor = ResultProcessor(
            config=mock_config,
            producer=mock_producer,
            inventory_table_path="abfss://test/xact_attachments",
        ...
        processor._inventory_writer = AsyncMock()
        processor._inventory_writer.write_results = AsyncMock(
        ...
        # Resolve — should handle exception gracefully
        await processor._resolve_pending_write()
