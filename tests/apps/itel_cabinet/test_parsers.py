...
from apps.itel_cabinet.parsers import (
    COLUMN_MAP,
    ApiResponse,
    DataBuilder,
    DateTimeEncoder,
...
class TestFromDict:
    ...
    def test_float_to_int_rounds_to_nearest(self):
        """Float values destined for int fields are rounded to the nearest whole number."""
        assert from_dict(int, 3.2) == 3
        assert from_dict(int, 3.7) == 4
        assert from_dict(int, 1.5) == 2

    def test_float_to_int_whole_number_unchanged(self):
        """Whole-number floats are converted to int without change."""
        assert from_dict(int, 3.0) == 3
        assert from_dict(int, 0.0) == 0

    def test_int_field_via_parse_task_data_round(self):
        """num_damaged_*_boxes float values are rounded to nearest in a full round-trip."""
        raw = {
            "assignmentId": 1,
            "taskId": 10,
            "taskName": "Cabinet Task",
            "projectId": 99,
            "formId": "form1",
                ...
                "groups": [
                    {
                        ...
                        "questionAndAnswers": [
                            {
                                "questionText": "Enter Number of Damaged Lower Boxes",
                                "component": "number",
                                "responseAnswerExport": {
                                    "type": "number",
                                    "numberAnswer": {"value": 2.3},
                                },
                                "formControl": {"id": "c1"},
                            }
                        ],
        ...
        api_obj = parse_task_data(raw)
        submission = parse_cabinet_form(api_obj, event_id="evt-1")
        assert submission.num_damaged_lower_boxes == 2
...
class TestDataBuilderBuildFormRow:
    ...
    def _make_available_group(self, group_name: str, question_text: str, answer: str) -> Group:
        return Group(
            name=group_name,
            group_id="g-avail",
            question_and_answers=[
                QuestionAndAnswer(
                    ...
                    response_answer_export=ResponseAnswerExport(
                        type="option",
                        option_answer=OptionAnswer(name=answer),
                    ),
                    form_control=FormControl(id="c-avail"),
                ),
            ],
        )

    def test_available_yes_stored_as_string(self):
        """'Yes' answer for *_available fields is stored as 'yes' string, not bool."""
        groups = [
            self._make_available_group(
                "Lower Cabinets",
                "Are All The Face Frames, Doors, And Drawers Available?",
                "Yes",
            )
        ]
        row = DataBuilder.build_form_row(self._make_api_response(groups=groups), "evt-1")
        assert row["lower_face_frames_doors_drawers_available"] == "yes"
        assert isinstance(row["lower_face_frames_doors_drawers_available"], str)

    def test_available_no_stored_as_string(self):
        """'No' answer for *_available fields is stored as 'no' string, not bool."""
        groups = [
            self._make_available_group(
                "Upper Cabinets",
                "Are All The Face Frames, Doors, And Drawers Available?",
        ...
        row = DataBuilder.build_form_row(self._make_api_response(groups=groups), "evt-1")
        assert row["upper_face_frames_doors_drawers_available"] == "no"
        assert isinstance(row["upper_face_frames_doors_drawers_available"], str)

    def test_available_not_available_maps_to_no(self):
        """'Not Available' answer maps to 'no' (not a distinct third value)."""
        groups = [
            self._make_available_group(
        ...
        row = DataBuilder.build_form_row(self._make_api_response(groups=groups), "evt-1")
        assert row["full_height_face_frames_doors_drawers_available"] == "no"

    def test_available_none_stays_none(self):
        """Missing *_available answer stays None (no default)."""
        row = DataBuilder.build_form_row(self._make_api_response(groups=[]), "evt-1")
        assert row["island_face_frames_doors_drawers_available"] is None

    def test_damaged_still_bool(self):
        """'Yes'/'No' answers for *_damaged fields remain bool (not converted to string)."""
        groups = [
            Group(
                name="Lower Cabinets",
                group_id="g-dmg",
                question_and_answers=[
                    QuestionAndAnswer(
                        question_text="Are there damages to the face frames, doors, and/or drawer fronts?",
                        component="option",
                        response_answer_export=ResponseAnswerExport(
                            type="option",
                            option_answer=OptionAnswer(name="Yes"),
                        ),
                        form_control=FormControl(id="c-dmg"),
                    ),
                ],
            )
        ]
        row = DataBuilder.build_form_row(self._make_api_response(groups=groups), "evt-1")
        assert row["lower_face_frames_doors_drawers_damaged"] is True
        assert isinstance(row["lower_face_frames_doors_drawers_damaged"], bool)
...
class TestParseCabinetForm:
    ...
    def _make_api_obj(self, **overrides):
        base = {
            "assignmentId": 1001,
            "taskId": 456,
        ...
        return parse_task_data(base)

    def test_basic_parsing(self):
        api_obj = self._make_api_obj()
        result = parse_cabinet_form(api_obj, "evt-1")
...
class TestParseCabinetAttachments:
    ...
    def test_empty_when_no_images(self):
        ...
        attachments = parse_cabinet_attachments(api_obj, "evt-1")
        assert attachments == []
...
class TestGetReadableReport:
    ...
    def test_public_api_empty_groups(self):
        ...
        report = get_readable_report(api_obj, "evt-1")
...
class TestColumnMap:
    ...
    def test_drawer_fronts_damaged_question_text(self):
        """Updated question text for drawer fronts damaged maps correctly for all cabinet groups."""
        question = "Are there damages to the face frames, doors, and/or drawer fronts?"
        expected = {
            "Lower Cabinets": "lower_face_frames_doors_drawers_damaged",
            "Upper Cabinets": "upper_face_frames_doors_drawers_damaged",
            "Full Height/Pantry Cabinets": "full_height_face_frames_doors_drawers_damaged",
            "Island Cabinets": "island_face_frames_doors_drawers_damaged",
        }
        for group, column in expected.items():
            assert (group, question) in COLUMN_MAP, f"Missing mapping for {group}"
            assert COLUMN_MAP[(group, question)] == column
